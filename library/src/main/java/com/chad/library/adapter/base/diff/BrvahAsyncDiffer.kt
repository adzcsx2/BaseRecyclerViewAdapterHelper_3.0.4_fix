...
class BrvahAsyncDiffer<T>(
    private val adapter: BaseQuickAdapter<T, *>, private val config: BrvahAsyncDifferConfig<T>
) : DifferImp<T> {
...
//        mReadOnlyList = Collections.unmodifiableList(adapterData);
    ...
    private fun latchList(
        newList: MutableList<T>, diffResult: DiffResult, commitCallback: Runnable?
    ) {
        val previousList: List<T> = adapter.data
        adapter.data = newList
    ...
    private fun onCurrentListChanged(
        previousList: List<T>, commitCallback: Runnable?
    ) {
